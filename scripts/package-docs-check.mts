...
function generateExtensionMatrixDocs(state: CatalogState): string {
  const lines: string[] = [
    ...
    "> Generated by `pnpm docs:catalog:write`. Do not edit this file by hand.",
    "",
    "This page lists Croco provider, integration, transport, and presentation adapter compatibility from `docs/package-catalog.json`. Required configuration, runtime support, package peer dependencies, maturity, and CI smoke status are intentionally separate so users can evaluate production readiness without treating a passing unit test as a maturity claim.",
    "",
...
function formatSmokeStatus(pkg: PackageRecord): string {
  return pkg.hasTests ? "entrypoint smoke + package tests" : "entrypoint smoke only";
}
