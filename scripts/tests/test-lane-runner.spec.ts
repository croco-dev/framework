...
describe("test lane runner", () => {
  ...
  it("resolves root vitest worker allocation dynamically up to runner parallelism", () => {
    expect(MAX_ROOT_VITEST_WORKERS).toBe(4);
    expect(DEFAULT_MAX_ROOT_VITEST_WORKERS).toBe(4);
    expect(resolveMaxRootVitestWorkers({}, 1)).toBe(1);
    expect(resolveMaxRootVitestWorkers({}, 2)).toBe(2);
    expect(resolveMaxRootVitestWorkers({}, 4)).toBe(4);
    expect(resolveMaxRootVitestWorkers({}, 8)).toBe(4);
    ...
    expect(resolveMaxRootVitestWorkers({ MAX_ROOT_VITEST_WORKERS: "0" }, 4)).toBe(4);
    expect(resolveMaxRootVitestWorkers({ CROCO_TEST_WORKERS: "-2" }, 4)).toBe(4);
  });
