...
export function buildReportMarkdown(report: PackageQualityReport): string {
  const lines = [
    ...
    `| \`provider-certification:check\` | provider, integration, transport, host, and presentation certification evidence | blocking on PR/trunk | ${report.gateOutcomes["provider-certification:check"]} | validates catalog certification records and writes \`ci-reports/package-quality/provider-certification.md\` plus JSON |`,
