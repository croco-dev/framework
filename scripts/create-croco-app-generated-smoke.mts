...
const nodeApplicationHostSmokeScript = [
  ...
  "const missingResponse = await fetch(`http://127.0.0.1:${address.port}/users/missing`);",
  "const missingProblem = await missingResponse.json();",
  'if (missingResponse.status !== 404 || missingProblem.code !== "starter/user-not-found") throw new Error(`Expected built Node host Problem filter, received ${JSON.stringify(missingProblem)}`);',
  "} finally {",
  "await running.close();",
  "}",
  "})();",
...
const adminNodeApplicationHostSmokeScript = [
  ...
  "const missingResponse = await fetch(`http://127.0.0.1:${address.port}/admin/users/missing`);",
  "const missingProblem = await missingResponse.json();",
  'if (missingResponse.status !== 404 || missingProblem.code !== "admin-console/user-not-found") throw new Error(`Expected built admin Node host Problem filter, received ${JSON.stringify(missingProblem)}`);',
  "} finally {",
  "await running.close();",
  "}",
  "})();",
      ...
      },
      {
        ...
        json: {
          ...
          matches: {
            ...
            billingUsageDrift: 0,
            aiQuotaFailureCode: "ai-usage/quota-exceeded",
