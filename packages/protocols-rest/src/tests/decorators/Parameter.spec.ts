...
describe('Parameter decorators', () => {
  ...
  describe('@Query decorator', () => {
    ...
    it('should register multiple query params', () => {
      ...
      class ItemController {
        @Get()
        list(@Query('page') page: string, @Query('limit') limit: string, @Query('sort') sort: string) {
  ...
  describe('@Header decorator', () => {
    it('should register header metadata with name', () => {
      ...
      const paramsMap = Reflect.getMetadata(REST_PARAMS_KEY, AuthController) as Map<string | symbol, ParamMetadata[]>;
    ...
    it('should register multiple headers', () => {
      ...
      const paramsMap = Reflect.getMetadata(REST_PARAMS_KEY, ApiController) as Map<string | symbol, ParamMetadata[]>;
  ...
  describe('Parameter index tracking', () => {
    it('should correctly track parameter indices', () => {
      ...
      class TestController {
        @Post()
        test(p1: string, @Param('id') p2: string, p3: string, @Body() p4: unknown, p5: string, @Query('q') p6: string) {
          return { p1, p2, p3, p4, p5, p6 };
        }
      }

      const paramsMap = Reflect.getMetadata(REST_PARAMS_KEY, TestController) as Map<string | symbol, ParamMetadata[]>;
