...
- 405633b: Reject invalid desktop authority, remote-origin, reference, and execution-limit contracts through deterministic build-time diagnostics.

### Minor Changes

- 4e4fbf7: Compile desktop application definitions into a deterministic, source-aware contract graph with a stable semantic hash and structured schema diagnostics.
- b62c1e1: Desktop commands now declare explicit Problem references, inherit standard Problems from effects, and compile renderer-safe failure unions against supplied Problem Registry manifests.
- 639abfe: Compile the fail-closed DesktopWire schema subset into deterministic descriptors and reject invalid wire values.
- 713d734: Classify desktop contract compatibility and renderer authority drift independently with stable review fingerprints and distinct exit status bits.
- f5f0b7d: Desktop applications can declare exact contract-derived command handler coverage through `app.implement()`.
...
- 87c48a3: Desktop applications can define typed commands, events, and local or remote windows without repeating IPC channel
  strings, with stable IDs and inference derived from contract object keys.
- 5312e2b: Desktop command handlers now receive an effect- and event-restricted context with typed success, Problem failure, and cancellation helpers.
- 423ca2a: Define opaque, typed desktop file and directory resource grants with deterministic metadata.

### Patch Changes

- ef11bcc: - test(protocols-desktop): keep effect fixtures contract-valid
- 28d9461: - fix: enforce desktop protocol contract boundaries
...
- 5833c83: - test(protocols-desktop): gate desktop type contracts at scale
