...
describe('MembershipManager', () => {
  ...
  describe('seat limit', () => {
    it('should throw SeatLimitExceededProblem when seat limit is exceeded', async () => {
      ...
      await expect(manager.addMember('tenant-1', 'user-1', 'member')).rejects.toBeInstanceOf(SeatLimitExceededProblem);
    });
  ...
  describe('removeMember', () => {
    ...
    it('should prevent removing the last owner', async () => {
      ...
      await expect(manager.removeMember('tenant-1', 'user-1')).rejects.toBeInstanceOf(LastOwnerCannotBeRemovedProblem);
    });
  ...
  describe('transferOwnership', () => {
    ...
    it('should throw OwnershipTransferRequiredProblem when from user is not owner', async () => {
      ...
      await expect(manager.transferOwnership('tenant-1', 'user-1', 'user-2')).rejects.toBeInstanceOf(
        OwnershipTransferRequiredProblem
      );
    });
