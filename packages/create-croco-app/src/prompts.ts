...
export async function runPrompts(cliArgs: Partial<GeneratorOptions>): Promise<GeneratorOptions> {
  ...
  const projectName =
    ...
    (await p.text({
      ...
      validate(value) {
        ...
        if (!/^[a-z0-9-_]+$/.test(value)) return 'Only lowercase letters, numbers, hyphens, underscores';
        return undefined;
      },
    }));
  ...
  if (preset === 'blank') {
    const installDeps = cliArgs.installDeps ?? (await p.confirm({ message: 'Install dependencies?' }));
