...
function checkSpaBeSplitStructure() {
  ...
  checkFileContains('spa-be-split', ['apps', 'api-server', 'src', 'index.ts'], /@croco\/transports-http/);
...
function checkContainerFullstackStructure() {
  ...
  const dockerfileContent = readFileSync(templatePath('container-fullstack', 'Dockerfile'), 'utf-8');
  ...
  checkFileContains('container-fullstack', ['apps', 'api-server', 'src', 'index.ts'], /\b(listen|createCrocoApp)\(/);
