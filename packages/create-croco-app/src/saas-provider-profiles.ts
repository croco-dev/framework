...
export function renderGeneratedSaasProviderProfileSource(
  ...
  return [
    ...
    'import { defineCrocoApplication, MODULE_CONTRIBUTION_KINDS, type ApplicationProviderReplacement, type ApplicationRuntimeGraphManifest, type CrocoApplicationDefinition, type CrocoPlugin, type ModuleOptions } from "@croco/framework-module";',
    ...
    "  readonly applicationModules?: (profileModules: readonly ModuleOptions[]) => readonly ModuleOptions[];",
    ...
    "    },",
    "  );",
    "  const profileModules = imports.flatMap((plugin) => plugin.modules);",
    "  const applicationModules = options.applicationModules?.(profileModules) ?? [];",
    "",
    "  return defineCrocoApplication({",
    "    name: generatedSaasProviderProfileManifest.profile.name,",
    "    imports: [...imports, ...applicationModules],",
    ...
    "  const otlpEndpoint = tracesEndpoint || (baseEndpoint",
    '    ? (baseEndpoint.endsWith("/") ? baseEndpoint : baseEndpoint + "/") + "v1/traces"',
    "    : undefined);",
    "  return {",
