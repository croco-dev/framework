...
import { defaultSaasRuntime, runSaasDemoFlow } from "../saasDemo";
...
async function main(): Promise<void> {
  ...
  const snapshot = await runSaasDemoFlow(defaultSaasRuntime);
  assertSaasSmokeContract(snapshot);

  const failed = await seedFailedBillingSyncJob();
  const running = await seedRunningUsageRollupJob();
  const {
    getJobExitCode,
    getJobsListExitCode,
    runJobsCancel,
  ...
  const replayExecution = await defaultSaasRuntime.executionManager.get(replayed.id);
...
async function seedFailedBillingSyncJob() {
  const execution = await defaultSaasRuntime.executionManager.create({
    type: "billing-sync",
  ...
  await defaultSaasRuntime.executionManager.start(execution.id);
  await defaultSaasRuntime.executionManager.recordLog(execution.id, {
    message: "Billing sync failed before provider checkpoint",
    data: { tenantId: "tenant_attention" },
    level: "warn",
  });
  return defaultSaasRuntime.executionManager.fail(execution.id, {
...
async function seedRunningUsageRollupJob() {
  const execution = await defaultSaasRuntime.executionManager.create({
    type: "usage-rollup",
    payload: { tenantId: "tenant_attention" },
    maxAttempts: 1,
