...
- f74f7d9: Subscribe to every distinct event declared with stacked `@RegisterEventHandler` decorators on one class.
...
- 1f3aeb7: Keep inherited event field metadata isolated so sibling event classes can reuse serialized field names without corrupting
  their parent.
