...
- e57cf64: Preserve every concurrent Upstash rate-limit counter increment and its configured TTL.
...
- 6dc3e49: Enforce Redis-backed quotas when fixed-window stores are called through the algorithm-specific contract.
- e21da38: Report sliding-window recovery from the oldest surviving Upstash entry after checks and refunds.
...
- 81c2daf: Keep fixed-window and token-bucket state and refund receipts in the same Redis Cluster slot so check and refund Lua scripts run without CROSSSLOT errors. Reset uses the same tagged keys, including when prefixes or caller keys contain braces or percent signs.
...
- 41aab7e: Upstash rate limit stores use an optional injected clock for window boundaries, Redis arguments, and reset times.
