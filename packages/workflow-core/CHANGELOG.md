...
- 4c34f96: Completed saga steps no longer retain an error from an earlier failed retry attempt.
- 5b441e2: Compensate saga steps whose work succeeded when the completion record cannot be stored.
...
- a94ce5f: Retry saga steps that throw standard JavaScript errors when a retry policy is configured, while preserving explicit non-retryable markers and custom retry decisions.
