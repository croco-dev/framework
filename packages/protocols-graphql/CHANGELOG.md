...
- 921fd58: - fix: enforce GraphQL contract smoke gates
...
- 5795e41: Execute declared GraphQL guards, role requirements, and interceptors through the Yoga resolver path with Croco DI and Problem responses.
...
- 6a39000: Normalize GraphQL decorator metadata ownership so contract snapshots and runtime policy readers use one inherited resolver-method contract without leaking derived declarations into base resolvers.
...
- 1049edf: Deny access without throwing when GraphQLRolesGuard has no valid metadata target. Explicit resolver targets and object or function roots retain role-based authorization.
