...
import { AiPricingRegistryConflictProblem } from "./problems/AiUsageProblems";
...
function createPricingFromRegistry(
  ...
  for (const entry of registry.entries) {
    ...
    if (providerPricing.has(entry.modelId)) {
      throw new AiPricingRegistryConflictProblem(entry.provider, entry.modelId, registry.version);
    }
...
export class AiPricingTable {
  ...
  static fromRegistry(registry: PricingRegistryDefinition): AiPricingTable {
    return new AiPricingTable(createPricingFromRegistry(registry), {
  ...
  public calculateCost(usage: AiUsageRecord, pricing: ModelPricing): number;
  public calculateCost(usage: AiEmbeddingUsageRecord, pricing: ModelPricing): number;
  public calculateCost(
    usage: AiUsageRecord | AiEmbeddingUsageRecord,
...
export const defaultAiPricingTable = AiPricingTable.fromRegistry(samplePricingRegistry);
