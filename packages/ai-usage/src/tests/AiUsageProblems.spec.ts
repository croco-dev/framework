...
describe("AiUsageProblems", () => {
  describe("AiUsageQuotaExceededProblem", () => {
    it("should create problem with quota exceeded details", () => {
      const problem = new AiUsageQuotaExceededProblem("llm.prompt_tokens", 15000, 10000);

      expect(problem.code).toBe("ai-usage/quota-exceeded");
    ...
    it("should serialize non-finite quota evidence as strings", () => {
      const problem = new AiUsageQuotaExceededProblem(
        "llm.prompt_tokens",
        Number.NaN,
  ...
  describe("AiCostLimitExceededProblem", () => {
    it("should create problem for daily limit exceeded", () => {
      const problem = new AiCostLimitExceededProblem("tenant-123", 15.5, 10.0, "daily");

      expect(problem.code).toBe("ai-usage/cost-limit-exceeded");
      expect(problem.category).toBe(ProblemCategory.Forbidden);
      expect(problem.status).toBe(403);
    ...
    it("should create problem for monthly limit exceeded", () => {
      const problem = new AiCostLimitExceededProblem("tenant-123", 150.0, 100.0, "monthly");
  ...
  describe("AiPricingNotFoundProblem", () => {
    it("should create problem with not found category", () => {
      const problem = new AiPricingNotFoundProblem("openai", "gpt-4");

      expect(problem.code).toBe("ai-usage/pricing-not-found");
  ...
  describe("AiUsageRecordFailedProblem", () => {
    it("should expose failed operation and meter ids", () => {
      const problem = new AiUsageRecordFailedProblem(
        "generate",
        ["llm.prompt_tokens", "llm.cost_usd_nanos"],
        new Error("boom"),
      );

      expect(problem.code).toBe("ai-usage/record-failed");
