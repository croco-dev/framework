...
- 7d1ed07: - fix(dataloader-core): deduplicate keys in batch when cache is disabled
...
- a9be396: Reject sparse batch results so every requested load settles and failed batch entries can be retried.
...
- f0c0b86: Reject fractional, unsafe, and otherwise invalid maximum batch sizes before a batch loader can enter dispatch.
- 3be86de: Preserve batching, cached results, and primed values outside a request context by retaining standalone instances per created loader and resolved scope.
