...
- dda0a50: Add a provider-neutral task dispatcher slot and canonical QStash task application plugin.
...
- 2575136: Preserve a task handler's successful result when it settles before a delayed timeout callback, even if the deadline has passed.
...
- f32c7f8: Retry ordinary task handler errors while attempts remain, preserve explicit non-retryable failures, allow each task to classify otherwise unmarked errors with `isRetryable`, and keep completion persistence failures outside handler retries.
...
- bc3cb67: Preserve the original task error when recording the failed execution also fails, while exposing the recording failure as secondary diagnostic evidence.
