...
describe('RedisUsageStorage', () => {
  ...
  describe('isIdempotent', () => {
    ...
    it('should throw RedisProblem on error', async () => {
      ...
      await expect(storage.isIdempotent('tenant-1', 'api_calls', 'key-123', 86400)).rejects.toThrow(RedisProblem);
    });
  });
  ...
  describe('time range calculation', () => {
    it('should use provided date range', async () => {
      ...
      expect(mockRedis.zrangebyscore).toHaveBeenCalledWith(expect.any(String), startDate.getTime(), endDate.getTime());
    });
  });
});
