...
describe('IdempotencyManager', () => {
  ...
  describe('checkAndMarkOrThrow', () => {
    it('should not throw for new key', async () => {
      ...
      await expect(manager.checkAndMarkOrThrow('tenant-1', 'api_calls', 'key-123')).resolves.not.toThrow();
    });

    it('should throw DuplicateRecordProblem for duplicate key', async () => {
      vi.mocked(mockRedis.set).mockResolvedValue(null);

      await expect(manager.checkAndMarkOrThrow('tenant-1', 'api_calls', 'duplicate-key')).rejects.toThrow(
