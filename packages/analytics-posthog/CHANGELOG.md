...
- f392e75: Use console logging when LOGGER_TOKEN is not registered so provider failures remain contained, disabled operations can complete, and flush failures retain their typed Problem.
...
- 14d7d42: Flush queued PostHog events without shutting down reusable clients, while keeping process shutdown as an explicit lifecycle operation.
- 4c9b8a1: Reject malformed and non-HTTP(S) PostHog hosts consistently during diagnostics and runtime initialization.
