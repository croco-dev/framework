...
- ed77a2b: Reject tenant-scoped operations when either supplied tenant identifier differs from the active tenant.
...
- b068efa: Keep tenant authorization outcomes authoritative when policy trace or isolation audit delivery fails by default, while exposing an explicit fail-closed mode for allowed decisions.
...
- cb53f45: Provide a tenant isolation enforcer that fails missing tenant context, default tenant fallback, unsafe cross-tenant queries, and unreasoned admin bypasses with typed Problems.
