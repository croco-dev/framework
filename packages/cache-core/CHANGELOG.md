...
- 6ff55f8: Ignore concurrently written entries that expire before a getOrSet loader completes, and cache the loaded value with its requested TTL.
...
- 5226e2e: Keep all-entry cache eviction within the configured namespace and preserve other cached entries. Clear the entire store only when no namespace is provided.
...
- 305eed7: Keep unrelated cache invalidations from suppressing concurrent `getOrSet()` loader results.
- 444a829: Expose an event-driven cache invalidation graph with manifest validation, adapter capability metadata, runtime invalidation, and test assertions for event-to-cache stale-read prevention.
