...
- f728a9f: Query monthly health usage across the complete UTC month with an explicit half-open interval.
- ec34601: Persist current and previous fractional health scores without integer truncation, and export the
  PostgreSQL migration that widens existing score columns while preserving integer rows.
- c2a2653: 건강 점수 스키마에 transition sequence 기본 키와 테넌트별 순번·계산 시각 인덱스를 선언합니다. 기존 DB는 README의 마이그레이션 SQL을 별도로 적용해야 합니다.
- b29827f: Join caller-owned transactions when storing customer health transitions and defer their external
  events until the transaction commits.
- e114d54: Keep committed health score history isolated from caller-owned inputs and query results while preserving assigned transition versions and nested signal dates across Drizzle JSON storage.
...
- Updated dependencies [377c684]
  ...
  - @croco/customer-health-core@1.0.0
