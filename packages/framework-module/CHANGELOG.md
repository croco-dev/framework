...
- da9925f: Complete every initialized module shutdown hook, aggregate ordered cleanup failures, and reset active runtime state after failed cleanup.
...
- 0efaa4f: Remove providers registered by the default module runtime when `resetModules()` resets module state.
...
- c0baec0: Define a first-class module contract with provider visibility, lifecycle failure Problems, shutdown ordering, and diagnostics metadata.
