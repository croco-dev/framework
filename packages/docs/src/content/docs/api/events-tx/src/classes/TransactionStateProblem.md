...
> **new TransactionStateProblem**(`detail`): `TransactionStateProblem`
...
> `readonly` **code**: `"events-tx/transaction-state-error"` = `"events-tx/transaction-state-error"`
