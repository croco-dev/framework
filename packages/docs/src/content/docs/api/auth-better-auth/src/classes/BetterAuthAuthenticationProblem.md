...
> **new BetterAuthAuthenticationProblem**(`operation`, `error`): `BetterAuthAuthenticationProblem`
...
> `readonly` **code**: `"auth-better-auth/authentication-failed"` = `"auth-better-auth/authentication-failed"`
