---
editUrl: false
next: false
prev: false
title: "InvalidOutboundWebhookSecretVersionProblem"
---

## Extends

- `OutboundWebhookProblem`

## Constructors

### Constructor

> **new InvalidOutboundWebhookSecretVersionProblem**(`endpointId`, `secretVersion`, `reason`): `InvalidOutboundWebhookSecretVersionProblem`

#### Parameters

##### endpointId

`string`

##### secretVersion

`string`

##### reason

`"unknown"` \| `"expired"`

#### Returns

`InvalidOutboundWebhookSecretVersionProblem`

#### Overrides

`OutboundWebhookProblem.constructor`

## Properties

### category

> `readonly` **category**: [`ProblemCategory`](/api/problems-core/src/enumerations/problemcategory/)

#### Inherited from

`OutboundWebhookProblem.category`

***

### cause?

> `readonly` `optional` **cause?**: `Error`

#### Inherited from

`OutboundWebhookProblem.cause`

***

### code

> `readonly` **code**: `string`

#### Inherited from

`OutboundWebhookProblem.code`

***

### detail?

> `readonly` `optional` **detail?**: `string`

#### Inherited from

`OutboundWebhookProblem.detail`

***

### extensions?

> `readonly` `optional` **extensions?**: [`ProblemExtensions`](/api/problems-core/src/type-aliases/problemextensions/)

#### Inherited from

`OutboundWebhookProblem.extensions`

***

### instance?

> `readonly` `optional` **instance?**: `string`

#### Inherited from

`OutboundWebhookProblem.instance`

***

### message

> **message**: `string`

#### Inherited from

`OutboundWebhookProblem.message`

***

### name

> **name**: `string`

#### Inherited from

`OutboundWebhookProblem.name`

***

### stack?

> `optional` **stack?**: `string`

#### Inherited from

`OutboundWebhookProblem.stack`

***

### type

> `readonly` **type**: `string`

#### Inherited from

`OutboundWebhookProblem.type`

***

### stackTraceLimit

> `static` **stackTraceLimit**: `number`

The `Error.stackTraceLimit` property specifies the number of stack frames
collected by a stack trace (whether generated by `new Error().stack` or
`Error.captureStackTrace(obj)`).

The default value is `10` but may be set to any valid JavaScript number. Changes
will affect any stack trace captured _after_ the value has been changed.

If set to a non-number value, or set to a negative number, stack traces will
not capture any frames.

#### Inherited from

`OutboundWebhookProblem.stackTraceLimit`

## Accessors

### status

#### Get Signature

> **get** **status**(): `number`

##### Returns

`number`

#### Inherited from

`OutboundWebhookProblem.status`

***

### title

#### Get Signature

> **get** **title**(): `string`

##### Returns

`string`

#### Inherited from

`OutboundWebhookProblem.title`

## Methods

### toJSON()

> **toJSON**(): [`ProblemDetails`](/api/problems-core/src/type-aliases/problemdetails/)

#### Returns

[`ProblemDetails`](/api/problems-core/src/type-aliases/problemdetails/)

#### Inherited from

`OutboundWebhookProblem.toJSON`

***

### captureStackTrace()

> `static` **captureStackTrace**(`targetObject`, `constructorOpt?`): `void`

Creates a `.stack` property on `targetObject`, which when accessed returns
a string representing the location in the code at which
`Error.captureStackTrace()` was called.

```js
const myObject = {};
Error.captureStackTrace(myObject);
myObject.stack;  // Similar to `new Error().stack`
```

The first line of the trace will be prefixed with
`${myObject.name}: ${myObject.message}`.

The optional `constructorOpt` argument accepts a function. If given, all frames
above `constructorOpt`, including `constructorOpt`, will be omitted from the
generated stack trace.

The `constructorOpt` argument is useful for hiding implementation
details of error generation from the user. For instance:

```js
function a() {
  b();
}

function b() {
  c();
}

function c() {
  // Create an error without stack trace to avoid calculating the stack trace twice.
  const { stackTraceLimit } = Error;
  Error.stackTraceLimit = 0;
  const error = new Error();
  Error.stackTraceLimit = stackTraceLimit;

  // Capture the stack trace above function b
  Error.captureStackTrace(error, b); // Neither function c, nor b is included in the stack trace
  throw error;
}

a();
```

#### Parameters

##### targetObject

`object`

##### constructorOpt?

`Function`

#### Returns

`void`

#### Inherited from

`OutboundWebhookProblem.captureStackTrace`

***

### prepareStackTrace()

> `static` **prepareStackTrace**(`err`, `stackTraces`): `any`

#### Parameters

##### err

`Error`

##### stackTraces

`CallSite`[]

#### Returns

`any`

#### See

https://v8.dev/docs/stack-trace-api#customizing-stack-traces

#### Inherited from

`OutboundWebhookProblem.prepareStackTrace`
