...
| -------------------------------------- | ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `@croco/frontend-react`                | Promoted to beta. | Package tests cover the `PageDataProvider`/`usePageData` render path. Generated `meta-vite-web` and `meta-vite-fullstack-workers` smoke assert browser hydration, visible hydration mismatch failure, page data/meta flow, and preserved hydration roots.          |
...
| `@croco/frontend-vite`                 | Promoted to beta. | Package tests cover Vite config helpers and optional Cloudflare peer diagnostics. Generated smoke now loads Vite config and builds browser output for SPA, `meta-vite-web`, and fullstack Worker profiles.                                                         |
| `@croco/transports-cloudflare-workers` | Promoted to beta. | Package tests cover Worker fetch dispatch, explicit env injection, runtime context propagation, and published Worker type surface. Generated fullstack Worker smoke exercises the service-bound API Worker path with no Cloudflare credentials.                    |
