...
| [`events-tx/inbox-processing-in-progress`](#events-tx-inbox-processing-in-progress)                                                   | Conflict              |    409 | conditional   | safe-message  | active     |       1 |
...
<a id="events-tx-inbox-processing-in-progress"></a>

## `events-tx/inbox-processing-in-progress`
...
- User action: Retry after extensions.lockedUntil; if it is absent, wait for operator reconciliation of the legacy processing record.
- Operator action: Check the inbox lease and outbox claim. Reconcile a processing record without a lease before redelivery.
- Telemetry: `croco.problem.warning` (warning) with `problem.code`, `problem.category`, `problem.status`

Sources:

- `packages/events-tx/src/libs/problems/EventsTxProblems.ts:163:1` (problem-class)
