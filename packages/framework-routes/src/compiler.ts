...
export function generateRouteRegistrationCode(controllers: readonly CompiledController[]): string {
  return generateModuleFromRouteRegistrationTable(
    createRouteRegistrationTable(controllers),
    "route compiled",
  );
}
...
export function generateModuleFromRouteRegistrationTable(
  ...
  lines.push("");
  lines.push("function createGeneratedRouteHandler(route) {");
  lines.push(`  return (c) => c.text(${JSON.stringify(responseText)});`);
  lines.push("}");
  lines.push("");
  lines.push("class RouteRegistrationTableError extends Error {");
  lines.push("  constructor(route) {");
  lines.push(
    "    super(`Unsupported generated route registration method ${route.method} for ${route.id}`);",
  );
  lines.push('    this.name = "RouteRegistrationTableError";');
  lines.push('    this.code = "route-registration-unsupported-method";');
  lines.push("  }");
  lines.push("}");
  lines.push("");
  lines.push("function registerGeneratedRoute(app, route) {");
  lines.push("  switch (route.method) {");
  lines.push('    case "GET":');
  lines.push("      app.get(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push('    case "POST":');
  lines.push("      app.post(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push('    case "PUT":');
  lines.push("      app.put(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push('    case "PATCH":');
  lines.push("      app.patch(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push('    case "DELETE":');
  lines.push("      app.delete(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push('    case "OPTIONS":');
  lines.push("      app.options(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push('    case "HEAD":');
  lines.push("      app.head(route.path, createGeneratedRouteHandler(route));");
  lines.push("      return;");
  lines.push("    default:");
  ...
  lines.push("    registerGeneratedRoute(app, route);");
  lines.push("  }");
  lines.push("}");
...
export async function compileRoutes(options: CompileRoutesOptions): Promise<void> {
  ...
  for (const controllerPath of options.controllerPaths) {
    controllerConstructors.push(...(await readControllerConstructors(controllerPath)));
  }
