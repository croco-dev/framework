...
- `RateLimiter`의 `failOpen` 옵션으로 직접 호출의 저장소 장애 허용 전략을 선택할 수 있습니다. HTTP
  `createRateLimitMiddleware`는 자신의 `failOpen` 정책을 각 limiter 체크에 명시적으로 전달하므로, limiter 생성자 기본값과
  달라도 미들웨어 정책이 저장소 장애 결과를 결정합니다.
