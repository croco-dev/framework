...
export class RenderServer {
  ...
  private async handleRsc(route: RenderRouteIR, request: Request, context?: RuntimeContext): Promise<Response> {
    try {
      const module = await route.componentLoader();
      const props = this.createComponentProps(request, context);
      const html = renderToString(createElement(module.default, props));
      const flightPayload = JSON.stringify({ nodeType: 'rsc-flight', path: route.path, content: html });
      const headMetadata = route.head?.();
  ...
  private createComponentProps(request: Request, context?: RuntimeContext): RenderRouteComponentProps {
    if (!context) {
      return { request };
    }
  ...
  private createRscErrorResponse(error: unknown, routePath: string): Response {
    ...
    return new Response(JSON.stringify({ error: 'RSC rendering failed', route: routePath, detail: message }), {
      status: 500,
