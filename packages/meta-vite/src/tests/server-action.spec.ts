...
import { z } from 'zod';
import { createServerAction, createServerActionHandler, dispatchServerAction } from '../libs/actions/serverActions';
...
describe('Server Actions', () => {
  it('registers and dispatches an action without schema', async () => {
    createServerAction({
      name: 'greet',
      handler: async (data) => {
        return new Response(JSON.stringify({ message: `Hello, ${(data as { name: string }).name}!` }), {
  ...
  it('dispatches an action with Zod schema validation', async () => {
    ...
    const response = await dispatchServerAction('submit-form', { email: 'test@example.com', count: 42 });
    expect(response.status).toBe(200);
...
describe('Server Action HTTP Integration', () => {
  it('handles POST /api/action/subscribe via composeHandler', async () => {
    createServerAction({
      name: 'subscribe',
      handler: async (data) => {
        return new Response(JSON.stringify({ subscribed: true, email: (data as { email: string }).email }), {
          headers: { 'Content-Type': 'application/json' },
        });
      },
    });
