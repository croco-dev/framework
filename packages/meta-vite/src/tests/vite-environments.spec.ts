...
describe('crocoMetaVitePlugin environments', () => {
  ...
  it('should preserve the ssr environment name exactly', () => {
    const plugin = getPlugin();
    const config = callConfig(plugin);
    const ssrEnvironmentName = Object.keys(config?.environments ?? {}).find((name) => name === 'ssr');
...
function callConfigEnvironment(plugin: Plugin, environmentName: string): EnvironmentOptions | null | undefined {
  ...
  const context = {} as never;
  if (typeof hook === 'object') {
    return hook.handler.call(context, environmentName, {}, createConfigEnv()) as EnvironmentOptions | null;
  }
...
function callResolveId(plugin: Plugin, environmentName: string, id: string): unknown {
  ...
  return hook?.call(createHookContext(environmentName) as never, id, undefined, { attributes: {}, isEntry: false });
}
