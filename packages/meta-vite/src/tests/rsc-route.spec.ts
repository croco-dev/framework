...
function isFlightPayload(value: unknown): value is FlightPayload {
  ...
  return typeof value.nodeType === 'string' && typeof value.path === 'string' && typeof value.content === 'string';
}
...
describe('RSC route rendering', () => {
  it('renders HTML with Flight payload', async () => {
    ...
    expect(payload).toEqual({ nodeType: 'rsc-flight', path: '/rsc-basic', content: '<main>RSC route: hello</main>' });
  });
    ...
    const server = createRegistryServer([
      {
        ...
        mode: 'rsc',
        component: () =>
          createElement('section', { 'data-client-component': 'BrowserEntry' }, createElement(BrowserEntry)),
      },
    ]);
  ...
  it('rejects server-only imports at route-level client boundaries', async () => {
    ...
    expect(leakyDiagnostic).toEqual({
      ...
      detail: 'Client boundary client-with-server-import.tsx imports server-only module(s): ./server-only-module',
    });
  });
});
