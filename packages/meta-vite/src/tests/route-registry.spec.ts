...
describe('ApiRoute registration', () => {
  ...
  it('throws RouteConflictError for duplicate path+method', () => {
    ...
    expect(() => registry.registerApiRoute(defineApiRoute({ path: '/api/users', handler }))).toThrow(
      RouteConflictError
    );
  });
  ...
  it('allows same path with different methods', () => {
    ...
    registry.registerApiRoute(defineApiRoute({ path: '/api/users', method: 'GET', handler: getHandler }));
    registry.registerApiRoute(defineApiRoute({ path: '/api/users', method: 'POST', handler: postHandler }));
  ...
  it('allows same method with different paths', () => {
    ...
    registry.registerApiRoute(defineApiRoute({ path: '/api/users', method: 'GET', handler: handler1 }));
    registry.registerApiRoute(defineApiRoute({ path: '/api/posts', method: 'GET', handler: handler2 }));
