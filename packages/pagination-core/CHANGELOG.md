...
- 1a8e54c: Infer compound cursor payload types from one Zod schema and validate the same contract during encoding and decoding.
...
- a2e7fab: Encode and decode cursor pagination tokens in browsers, workers, and edge runtimes without a Node `Buffer` polyfill while preserving existing base64url cursor strings.
...
- 68352be: Normalize parser and schema pagination numbers with the same whole-string, flooring, defaulting, and clamping policy.
