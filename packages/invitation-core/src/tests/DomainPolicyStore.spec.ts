...
describe('InMemoryDomainPolicyStore', () => {
  ...
  it('should update existing policy when saving same tenant and domain', async () => {
    await store.save(createPolicy({ id: 'dp-1', tenantId: 'tenant-1', domain: 'croco.dev', role: 'member' }));
    await store.save(createPolicy({ id: 'dp-2', tenantId: 'tenant-1', domain: 'croco.dev', role: 'viewer' }));
