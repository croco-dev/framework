...
describe('InMemoryInvitationStore', () => {
  ...
  it('should find invitation by tenant and email', async () => {
    await store.save(createInvitation({ id: 'inv-1', tenantId: 'tenant-1', email: 'member-1@croco.dev' }));
    await store.save(createInvitation({ id: 'inv-2', tenantId: 'tenant-2', email: 'member-1@croco.dev' }));
  ...
  it('should return all invitations by tenant', async () => {
    ...
    expect(invitations.map((invitation: Invitation) => invitation.id).sort()).toEqual(['inv-1', 'inv-2']);
  });
