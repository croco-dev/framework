...
- fd10325: Preserve plain-text and empty HTTP exception filter responses when exposing tRPC errors.
- 67e0cbe: fix: resolve published package types before runtime conditions
- f24c269: Preserve supported Croco client and upstream Problem classifications when converting failures to tRPC errors.
...
- 8f8ac0e: Validate contract-declared bodies in tRPC input envelopes even when the controller has no body parameter.
- e3a225f: Normalize HTTP/2 authority into the request Host header when Host is absent, preserving existing Host precedence.
- e07658f: Normalize Node HTTP/2 requests without copying pseudo-headers into Fetch headers, and use string `:authority` values when the Host header is absent.
