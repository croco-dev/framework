...
- 4137d6d: Lifecycle action dispatch now atomically claims idempotency and cooldown windows, holds an
...
- f187abe: - fix(lifecycle-core): hold execution lease until async actions settle
- 543c1fa: Distinguish lifecycle runs whose actions all skip from runs that produce no actions.
