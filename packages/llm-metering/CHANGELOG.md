...
- 6c8cd4f: Record `@AiMetered` token and cost usage when stream consumers stop early or throw, while preserving provider errors and reporting metering failures.
...
- 9d14704: Count repeated AiMetered calls independently with unique default invocation keys and resolve tenant identity from request Context before instance defaults. Explicit tenant options and custom idempotency key extractors retain precedence.
- 6efe63e: Honor custom generation and embedding usage extractors in `@AiMetered` while preserving automatic usage detection when they do not return usage.
...
- 6df614c: - Clarify LLM metering API documentation for pricing registry metadata, quota policy context, and usage meter deltas.
