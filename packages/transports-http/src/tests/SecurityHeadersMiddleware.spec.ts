...
import { type SecurityHeadersOptions, securityHeadersMiddleware } from '../libs/middleware/SecurityHeadersMiddleware';
...
describe('securityHeadersMiddleware', () => {
  it('should add all security headers by default', async () => {
    ...
    expect(ctx.raw.header).toHaveBeenCalledWith('Strict-Transport-Security', expect.stringContaining('max-age='));
    ...
    expect(ctx.raw.header).toHaveBeenCalledWith('Referrer-Policy', 'strict-origin-when-cross-origin');
  });
  ...
  it('should set correct HSTS header with includeSubDomains', async () => {
    ...
    expect(ctx.raw.header).toHaveBeenCalledWith('Strict-Transport-Security', 'max-age=31536000; includeSubDomains');
  });
  ...
  it('should disable Strict-Transport-Security when option is false', async () => {
    ...
    expect(ctx.raw.header).not.toHaveBeenCalledWith('Strict-Transport-Security', expect.any(String));
  });
