import { context, propagation, type Span, type SpanOptions, trace } from '@opentelemetry/api';
...
describe('TelemetryMiddleware', () => {
  ...
  it('should create the server span from propagation-extracted context', async () => {
    const ctx = createContext(new Headers({ traceparent: '00-11111111111111111111111111111111-2222222222222222-01' }));
    ...
    const tracer = {
      startSpan,
      startActiveSpan: vi.fn(<T>(_name: string, fn: (activeSpan: typeof span) => T, _options?: SpanOptions) =>
        fn(span)
      ),
    };
    ...
    expect(propagation.extract).toHaveBeenCalledWith(context.active(), ctx.req.headers, expect.any(Object));
