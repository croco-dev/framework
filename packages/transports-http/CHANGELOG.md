...
- bebc2f6: Health and readiness checks now cancel active indicators when their caller aborts, including HTTP health requests whose clients disconnect.
...
- d9ab40a: - Keep declared route success statuses consistent across extraction, OpenAPI responses, and HTTP output.
...
- e5aee59: Lambda handlers base64-encode responses that carry a non-identity `Content-Encoding` or a body that is not valid UTF-8, so compressed responses reach clients byte-for-byte.
...
- 1d935c2: Preserve `@Ctx()` and `@Raw()` framework context parameters when HTTP pipes are active, and omit their non-executing pipe stages from request pipeline graphs.
