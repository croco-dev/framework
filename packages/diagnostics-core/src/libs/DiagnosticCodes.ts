...
export const CROCO_DIAGNOSTIC_CODE_DEFINITIONS = [
  ...
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_006",
    category: "build-time",
    title: "Workspace package version is inconsistent",
    cause:
      "A workspace package depends on another workspace package without using the workspace protocol.",
    action: "Use workspace:* for local workspace package dependencies or align the package range.",
    legacyCodes: ["doctor/workspace-version-inconsistent"],
    searchKeywords: ["croco doctor", "workspace dependency", "workspace protocol"],
    fixExample: { label: "Normalize package manifests", command: "pnpm package-manifests:write" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_007",
    category: "build-time",
    title: "Spine package is missing",
    cause: "A Croco spine package required by the app or package catalog is not installed.",
    action: "Install dependencies or add the missing spine package to the workspace.",
    legacyCodes: ["doctor/spine-package-missing"],
    searchKeywords: ["croco doctor", "spine package", "install state"],
    fixExample: { label: "Install dependencies", command: "pnpm install" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_008",
    category: "build-time",
    title: "Spine package build output is missing",
    cause: "A Croco spine package is present but its dist build output is missing.",
    action: "Build the spine package set before running release or generated-app readiness checks.",
    legacyCodes: ["doctor/spine-package-unbuilt"],
    searchKeywords: ["croco doctor", "spine package", "dist", "build state"],
    fixExample: { label: "Build the workspace", command: "pnpm build" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_009",
    category: "build-time",
    title: "Contract Graph snapshot is missing",
    cause: "The app declares contract scripts but no Contract Graph snapshot artifact is present.",
    action: "Run the app's contract snapshot or contract verify script and commit the artifact.",
    legacyCodes: ["doctor/contract-graph-missing"],
    searchKeywords: ["croco doctor", "contract graph", "snapshot missing"],
    fixExample: { label: "Write contract snapshot", command: "pnpm contract:snapshot" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_010",
    category: "build-time",
    title: "Contract Graph snapshot is invalid",
    cause: "The Contract Graph snapshot is not valid croco.contract-graph.snapshot.v1 JSON.",
    action: "Regenerate the Contract Graph snapshot from current controller sources.",
    legacyCodes: ["doctor/contract-graph-invalid"],
    searchKeywords: ["croco doctor", "contract graph", "invalid snapshot"],
    fixExample: { label: "Regenerate contract snapshot", command: "pnpm contract:snapshot" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_011",
    category: "build-time",
    title: "Contract Graph snapshot contains errors",
    cause: "The Contract Graph snapshot includes one or more error diagnostics.",
    action: "Fix the route contract diagnostics and regenerate contract artifacts.",
    legacyCodes: ["doctor/contract-graph-errors"],
    searchKeywords: ["croco doctor", "contract graph", "diagnostics"],
    fixExample: { label: "Verify contracts", command: "pnpm contract:verify" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_012",
    category: "build-time",
    title: "Problem registry artifact is missing",
    cause: "The workspace contains Problem registry sources but no generated registry artifact.",
    action: "Run the Problem registry write command and commit the generated artifact.",
    legacyCodes: ["doctor/problem-registry-missing"],
    searchKeywords: ["croco doctor", "problem registry", "missing"],
    fixExample: { label: "Write Problem registry", command: "pnpm problem-registry:write" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_013",
    category: "build-time",
    title: "Problem registry artifact is invalid",
    cause: "The generated Problem registry artifact is not valid registry JSON.",
    action: "Regenerate the Problem registry artifact and inspect the produced diff.",
    legacyCodes: ["doctor/problem-registry-invalid"],
    searchKeywords: ["croco doctor", "problem registry", "invalid"],
    fixExample: { label: "Check Problem registry", command: "pnpm problem-registry:check" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_014",
    category: "build-time",
    title: "Problem registry artifact has drift",
    cause: "The generated Problem registry counts or unique code set are internally inconsistent.",
    action: "Regenerate the Problem registry artifact and commit the synchronized output.",
    legacyCodes: ["doctor/problem-registry-drift"],
    searchKeywords: ["croco doctor", "problem registry", "drift"],
    fixExample: { label: "Refresh Problem registry", command: "pnpm problem-registry:write" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_015",
    category: "build-time",
    title: "Runtime capability manifest is missing",
    cause: "The app references runtime policy checks but no runtime capability manifest exists.",
    action: "Generate croco-runtime-policy.manifest.json for the app runtime target.",
    legacyCodes: ["doctor/runtime-capability-manifest-missing"],
    searchKeywords: ["croco doctor", "runtime capability", "manifest missing"],
    fixExample: { label: "Check runtime policy", command: "pnpm runtime-policy:check" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_016",
    category: "build-time",
    title: "Runtime capability manifest is invalid",
    cause: "The runtime capability manifest is not valid JSON or lacks a runtime target.",
    action: "Add runtime.platform or target and ensure the policy table is valid JSON.",
    legacyCodes: ["doctor/runtime-capability-manifest-invalid"],
    searchKeywords: ["croco doctor", "runtime capability", "manifest invalid"],
    fixExample: { label: "Check runtime policy", command: "pnpm runtime-policy:check" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_017",
    category: "runtime",
    title: "HTTP security validation is disabled",
    cause: "A Croco HTTP app explicitly disables security middleware validation.",
    action: "Configure the required HTTP security middleware and remove securityValidation: 'off'.",
    legacyCodes: ["doctor/http-security-validation-disabled"],
    searchKeywords: ["croco doctor", "HTTP security", "securityValidation"],
    fixExample: { label: "Inspect HTTP bootstrap", command: "pnpm exec croco doctor" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_018",
    category: "runtime",
    title: "HTTP security middleware is incomplete",
    cause: "A Croco HTTP app is missing one or more required security middleware registrations.",
    action:
      "Register securityHeadersMiddleware, corsMiddleware, bodyLimitMiddleware, and rateLimitHttpMiddleware.",
    legacyCodes: ["doctor/http-security-middleware-missing"],
    searchKeywords: ["croco doctor", "HTTP security", "middleware missing"],
    fixExample: { label: "Inspect HTTP bootstrap", command: "pnpm exec croco doctor" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_019",
    category: "dependency-injection",
    title: "DI graph manifest is missing",
    cause: "The app references DI graph checks but no DI graph manifest artifact exists.",
    action: "Generate or provide croco.di-graph.manifest.json before running doctor.",
    legacyCodes: ["doctor/di-graph-manifest-missing"],
    searchKeywords: ["croco doctor", "DI graph", "manifest missing"],
    fixExample: { label: "Check DI graph", command: "pnpm exec croco di check" },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_020",
    category: "dependency-injection",
    title: "DI graph bootstrap has errors",
    cause: "The DI graph manifest reports failed status or error diagnostics.",
    action: "Fix the DI bootstrap diagnostics and regenerate the DI graph manifest.",
    legacyCodes: ["doctor/di-graph-bootstrap-failed"],
    searchKeywords: ["croco doctor", "DI graph", "bootstrap errors"],
    fixExample: {
      label: "Inspect DI graph report",
      command: "pnpm exec croco di check croco.di-graph.manifest.json --json",
    },
  }),
  createCliDiagnosticCodeDefinition({
    code: "CROCO_CLI_DOCTOR_021",
    category: "build-time",
    title: "Provider certification evidence has gaps",
    cause:
      "The provider profile manifest is missing required capability evidence or package declarations.",
    action: "Complete the provider profile manifest or declare the packages it certifies.",
    legacyCodes: ["doctor/provider-certification-gap"],
    searchKeywords: ["croco doctor", "provider profile", "certification gap"],
    fixExample: { label: "Check provider profile", command: "pnpm profile:check" },
  }),
  createCliDiagnosticCodeDefinition({
