...
- 934f52b: - fix(scripts): exclude partially skipped tests from completed evidence
...
- 7c9a601: Use schema-qualified migration metadata tables consistently across detection and migration record operations.
...
- 88494b8: Preview selected up and down migrations through `--dry-run` or the explicit runner preview APIs without committing checkpoint or migration state, with deterministic recovery guidance when execution fails.
- a4a5a49: Load migration files from relative, absolute, spaced, and Windows filesystem directories through file URLs.
- 3f14cf9: Reject migration status, execution, rollback, and preview decisions when applied migration history no longer matches the available files.
- c9b2e11: Reject malformed persisted migration metadata before status, execution, preview, or rollback decisions.
...
- 82ce9a0: - fix: report fresh database migrations as pending without creating migration metadata
