...
describe("CloudinaryProvider", () => {
  ...
  describe("getStream()", () => {
    it.each(["uploads/avatar_123", "uploads/avatar.", "uploads/avatar.png."])(
      "should request an automatic format for extensionless image key %s",
      async (key) => {
        vi.mocked(global.fetch).mockResolvedValue(new Response(new Uint8Array(10)));
        ...
        expect(global.fetch).toHaveBeenCalledWith(
          `https://res.cloudinary.com/test-cloud/image/upload/f_auto/${key}`,
          { signal: undefined },
        );
      },
    );
