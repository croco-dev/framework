...
- a57d513: Continue publishing independent outbound webhook intents after retryable or terminal failures and
...
- 8b75dd8: Preserve webhook idempotency when equivalent UTF-8 bodies arrive as strings or binary buffers.
- 482ada3: Keep outbound webhook retry schedules consistent by rejecting invalid retry timing and clearing
  scheduled attempts when deliveries become terminal.
- b08f0db: Reject duplicate webhook event handler registrations without replacing the first handler.
- ce559af: Reject outbound webhook signatures whose signed Unix timestamp is malformed or outside the five-minute replay window.
- 2b1961e: Publish outbound webhook events independently of older failed dispatch intents. Duplicate event publication retries only that event's unpublished intents, while retained failures remain available through the explicit backlog drain.
...
- a251212: Reset a replayed outbound webhook delivery's retry budget, keep replay retry dispatches idempotently distinct, and
  preserve its recorded attempt history.
- 20c9891: Outbound webhook URL validation rejects NAT64 and 6to4 addresses that embed blocked IPv4 targets, and rejects site-local IPv6 addresses.
- eb13a80: Block uncompressed, alternative, and IPv4-mapped IPv6 loopback, local, and metadata addresses in outbound webhook URL SSRF validation.
- 67a0380: Default outbound webhook retries now wait 60, 120, then 240 seconds after consecutive failures.
...
- Updated dependencies [0502f95]
- Updated dependencies [584bd0e]
...
- cb425f3: Provide a generic webhook gateway contract for signature verification, idempotent dispatch/replay, unknown event policy, typed handler routing, and local fixture replay.

### Patch Changes

- Updated dependencies [aaf7675]
