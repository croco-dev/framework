...
describe("CarryingCapacityCalculator", () => {
  ...
  describe("calculateUserCC", () => {
    ...
    it("should support tenant-specific calculation", async () => {
      ...
      expect(mockUserProvider.getNewUsersCount).toHaveBeenCalledWith(
        expect.any(Date),
        "tenant-123",
      );
      expect(result).not.toBeNull();
  ...
  describe("calculateRevenueCC", () => {
    ...
    it("should calculate Revenue CC correctly with monthly new MRR=30000, NRR=98%", async () => {
      ...
      const result = await calculator.calculateRevenueCC({
        lookbackMonths: 2,
        tenantId: TENANT_ID,
      });
      ...
      expect(result.headroomPercent).toBeCloseTo(
        ((expectedCapacity - 500000) / expectedCapacity) * 100,
        1,
      );
    });
  ...
  describe("retention failures", () => {
    it("should propagate retention metric failures instead of using fabricated values", async () => {
      ...
      ).rejects.toBeInstanceOf(RetentionMetricsUnavailableProblem);
    });
  ...
  describe("simulate", () => {
    ...
    it("should simulate both churn and inflow changes", async () => {
      const result = await calculator.simulate({
        tenantId: TENANT_ID,
        inflowChange: 20,
        churnChange: -20,
      });
    ...
    it("should calculate correct headroom percent delta", async () => {
      ...
      expect(result.headroomPercentDelta).toBeCloseTo(
        simulatedHeadroomPercent - baselineHeadroomPercent,
        1,
      );
    });
  });
  ...
  describe("buildCCResult", () => {
    it("should calculate headroom and headroom percent correctly", async () => {
      ...
      const expectedCapacity = 1000 / (-Math.log(0.98) / 30);
      vi.spyOn(mockUserProvider, "getDailyActiveUsers").mockResolvedValue(
        Math.floor(expectedCapacity),
      );
