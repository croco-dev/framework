...
describe('OutputContractValidator', () => {
  ...
  it('reports warning when entry references missing artifact', () => {
    ...
    expect(
      report.results.some((result) => result.message.includes('index.d.ts') && result.severity === 'warning')
    ).toBe(true);
  });
  ...
  it('handles empty contract gracefully', () => {
    const report = validator.validate({ presetName: '', buildTime: '', format: 'esm', artifacts: [], entries: [] });
