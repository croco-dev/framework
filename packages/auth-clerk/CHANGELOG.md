...
- f58d722: Accept Clerk deleted-object payloads for user and organization deletion webhooks.
...
- 3d49e5d: Revoke only active Clerk sessions and continue bulk revocation when a listed session has already ended or disappeared, while preserving upstream failures that do not confirm session termination.
- fe50e0a: Forward organization membership pagination options to Clerk so callers can retrieve the requested page.
- a40573f: Keep organization membership counts consistent with the filtered memberships returned by the Clerk adapter.
...
- 8fa56cb: Revoke every Clerk session for a user even when the session list spans multiple API pages.
- 2a39372: Normalize every Clerk user, organization, membership, invitation, and session SDK failure into a retry-classified Croco Problem without exposing provider request or credential details.
- Updated dependencies [ed77a2b]
