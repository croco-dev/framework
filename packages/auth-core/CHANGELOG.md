...
- d53e75a: Verify that scoped manage implication remains limited to the same resource ID across read, write, delete, and
  manage checks.
...
- ae26c9d: Return an authentication-required Problem when a permission-protected route has no authenticated principal.
- b94097f: Preserve human-readable details when serializing authentication, rate-limit, retry, and GraphQL transport Problems.
...
- 1b8b1d6: Prevent object-scoped RBAC grants, including `manage`, from satisfying global or differently scoped permission requirements, and reject empty resource identifiers consistently.
- d49daac: Enforce `@RequireApiKey` routes as API-key-only so ordinary user authentication cannot cross the declared credential boundary.
- 4daea01: Reject permission objects that cannot round-trip through the parser while preserving the existing stable validation Problems.
