...
function writeConsumer(consumerPath: string): void {
  writeFileSync(
    consumerPath,
    [
      'import { InMemorySearchTransformRegistry, SearchTransformAdapter, Searchable, SearchableIndexConflictProblem } from "@croco/search-core";',
      "class ExternalTransformAdapter extends SearchTransformAdapter {",
      '  id = "external.uppercase";',
      '  defaultSuffix = "_uppercase";',
      "  transform(input, options) { return options.prefix + input.toUpperCase(); }",
      "}",
      "const transforms = new InMemorySearchTransformRegistry();",
      "const transformRef = transforms.register(new ExternalTransformAdapter());",
      'if (transforms.apply(transformRef, "croco", { prefix: ">" }) !== ">CROCO") {',
      '  process.stderr.write("Expected the external transform adapter to execute\\n");',
      "  process.exit(1);",
      "}",
      "class AlphaEntity {}",
      "class ZetaEntity {}",
