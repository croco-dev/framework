...
- 4090168: Reject failed automatic index and delete deliveries after publishing the failure event so event infrastructure can retry them and recover the search index.
- 88ef2f7: - fix: make search auto-sync enforce event tenant identity and report conflicts as `search-core/sync-identity-conflict`
...
- 3e976a2: Expose failed search sync event publish errors through a documented best-effort logging contract with event context.
