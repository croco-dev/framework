...
function writeConsumerTypecheck(consumerRoot: string): void {
  writeFileSync(
    ...
    [
      ...
      "});",
      "",
      "const Transformed = defineMessage({",
      '  id: "billing.transformed",',
      '  topic: "billing",',
      "  data: z.object({ name: z.string().transform((value) => value.length) }).strict(),",
      '  channels: ["email"],',
      "});",
      'const transformedInput: MessageDataInput<typeof Transformed> = { name: "Ada" };',
      "const transformedOutput: MessageData<typeof Transformed> = { name: 3 };",
      "void transformedInput; void transformedOutput;",
      "",
      "@Renders(TrialEnding)",
      ...
      'engagement.send(Transformed, { recipient: { tenantId: "tenant-1", userId: "user-1" }, data: transformedInput, key: "transformed-1" });',
      "engagement.send(Transformed, {",
...
function writeRuntimeConsumers(consumerRoot: string): void {
  const source = [
    ...
    'engagement.send(message, { recipient: { tenantId: "tenant-1", userId: "user-1" }, data: { name: "Ada" }, key: "message-1" }).then((result) => { if (result.status !== "queued" || result.executionIds[0] !== "execution-1" || transformations !== 1 || dispatchedPayload?.subject !== "3") throw new Error("packed engagement send failed"); });',
  ];
  writeFileSync(
