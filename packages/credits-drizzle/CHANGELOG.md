...
- eb7e702: Claim credit outbox events atomically so concurrent workers and command replays share one publication lease. Expired leases can be reclaimed, and stale workers cannot acknowledge or release replacement claims.
...
- a23cdde: Scope credit idempotency records, advisory locks, and durable event intents by tenant so independent ledgers can safely reuse client-generated keys.

### Patch Changes

- 0af349c: Failed credit allocations leave grant lots unchanged when callers handle the failure and commit an ambient transaction.
...
- 26739aa: Keep refunded credits spendable when their original grant lots have expired. Refund lots retain the earliest original expiry still in the future, or have no expiry when none remains, while preserving meter restrictions and allocation history.
...
- bb6266e: Keep committed credit commands successful when documented transaction publishers must retain event
  intents for idempotent recovery.
