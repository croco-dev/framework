...
- 4caf84c: Select audited resource IDs and payloads by validated parameter indexes so reordered and optional handler arguments cannot be misattributed.
...
- 9daa586: Attribute impersonated HTTP audit entries to the administrator and preserve the target user in audit metadata.
- 9c57a51: Attribute authenticated HTTP audit entries to the request principal and use its tenant when no request-context tenant exists.
...
- 69dbcff: Mark `@Auditable` handlers for global interceptor coordination so each invocation emits one audit entry.
- 67e0cbe: fix: resolve published package types before runtime conditions
- b097462: Preserve the original handler failure when persisting its audit entry also fails, while retaining the audit-write failure as diagnostic evidence.
- 5d54fb4: declare Apache-2.0 license across all publishable package manifests and ship LICENSE in published packages
- 1d5cfe1: Sanitize interceptor audit payloads and HTTP metadata before persistence, and preserve successful handler responses when audit storage fails while recording the failure through telemetry.
- 4ce0990: Redact common credential fields throughout decorator audit payloads and require successful result capture to be explicitly enabled.
- be7b62e: Block audited method execution when throwOnFailure is enabled and audit dependencies cannot be resolved.
- 92b2a8b: Stop pending backoff timers and subsequent audit write retries when the fire-and-forget handle is aborted. In-flight operations retain their successful result; cancellation does not report retry exhaustion.
