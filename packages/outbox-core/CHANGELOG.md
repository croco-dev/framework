...
- 0502f95: Delay retryable in-memory outbox failures by one second when dispatchers omit an explicit next-visible time.
- 584bd0e: Keep tenant idempotency and claim boundaries distinct when identity components contain delimiters.
...
- f303fcb: Stop reclaiming expired in-memory outbox records after their retry budget is exhausted, persist terminal failure evidence, and allow healthy records to continue through the queue.
...
- 361789b: Define the provider-neutral transactional outbox store contract with conformance tests.
