...
describe('InMemoryStorageProvider', () => {
  ...
  describe('put()', () => {
    ...
    it('/로 시작하는 키로 업로드 시도 시 InvalidKeyProblem throw', async () => {
      await expect(provider.put('/invalid/key', Buffer.from('data'))).rejects.toThrow(InvalidKeyProblem);
    });
  ...
  describe('getSignedUrl()', () => {
    ...
    it('존재하지 않는 파일로 서명된 URL 생성 시도 시 FileNotFoundProblem throw', async () => {
      await expect(provider.getSignedUrl('nonexistent/file.txt', { expiresIn: 3600 })).rejects.toThrow(
        FileNotFoundProblem
      );
    });

    it('유효하지 않은 키로 서명된 URL 생성 시도 시 InvalidKeyProblem throw', async () => {
      await expect(provider.getSignedUrl('invalid//key', { expiresIn: 3600 })).rejects.toThrow(InvalidKeyProblem);
    });
  });
  ...
  describe('getMetadata()', () => {
    ...
    it('존재하지 않는 파일 메타데이터 조회 시 FileNotFoundProblem throw', async () => {
      await expect(provider.getMetadata('nonexistent/file.txt')).rejects.toThrow(FileNotFoundProblem);
    });
