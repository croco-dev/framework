...
- c3de6cc: For allowed origins, apps with CORS middleware now answer preflight requests for registered routes without an explicit `@Options` handler. The default CORS configuration also allows requested headers such as `content-type` and varies the preflight response by `Access-Control-Request-Headers`.
...
- d0ed66c: Execute bounded retries for classified transient search and storage provider failures while preserving abort boundaries.
...
- 3434d4b: Continue cache invalidation after individual adapter failures and report every failed operation.
- fb1faf9: - fix(triggers-qstash): enforce cron execution deadlines and expose cooperative cancellation context
...
- f438532: Reject duplicate onboarding steps and unsupported dependency or feature-flag policies at registration. Align the public analytics event types with the two emitted names and payloads; consumers of `OnboardingEvent` must migrate from the former un-emitted names and context fields.
...
- 0b5a8fa: Generate signed Cloudinary downloads through the authenticated download API so the returned URL is rejected after its requested expiry while public delivery URLs remain unchanged.
...
- eb8003d: Mark CORS responses as varying by Origin while preserving existing Vary headers, and keep Problem Registry source locations current.
...
- 7d9c92b: Reject non-positive or non-finite NRR and non-finite user carrying capacity calculations with a typed Problem.
...
- 2bcfa27: - Record handled HTTP 5xx errors in diagnostics history while excluding 4xx responses, expose their stable diagnostic code, and keep the generated Problem registry aligned.
...
- 163b65c: Expose committed event publication failures through `publishAfterCommit` error callbacks and a stable Problem code.
...
- 3f99747: Allow callers to abort an in-memory publish waiting for a handler slot through the event publishing contract and facade, with generated Problem source locations kept in sync.
...
- 0530556: Keep QStash delivery identity API documentation aligned with the verification input and callback contracts.
...
- 1884ceb: Reject a different `createBatchLoader()` result that reuses another loader's name and scope within one request with `DuplicateBatchLoaderNameProblem` (`dataloader-core/duplicate-loader-name`), instead of silently returning the first loader's batches and cached results. Repeated calls to the same loader, `BatchLoaderFactory.create()` retrieval by name, and loaders outside a request context are unchanged; create each `createBatchLoader()` loader once and reuse it rather than recreating it per call inside a request. Custom `IBatchLoaderFactory` implementations that are called per invocation, such as through `@BatchLoad`, should delegate to `BatchLoaderFactory` instead of `createBatchLoader()`. Request cache keys now encode name, scope, and dynamic scope unambiguously, so different loaders whose name and scope strings only matched after delimiter concatenation no longer share a cache.
...
- ce1a95b: Map Croco REST request body validation issues to form field errors in frontend and generated RPC clients. Update the generated Problem registry source location.
...
- 2742cbc: Return a typed recovery Problem when an idempotent task call finds a failed, cancelled, or timed-out execution instead of exposing an execution state-machine error, and publish its generated registry entry.
- 555d5fe: Expose composable Drizzle database capability contracts, use them across adapters, and replace the ambiguous audit and metering `DrizzleDb` exports with adapter-specific database types.
...
- 9163f70: Preserve PostgreSQL usage record identities, encode Drizzle timestamps for each supported dialect, and keep generated
  problem-code source locations aligned with the metering implementation.
...
- 2c68e9c: Reject request bodies with unsupported media types as RFC 7807 415 Problems before reading their bytes.
...
- b242060: Typed product events validate before local delivery and expose a deterministic, permission-gated event catalog with isolated payload testing.
