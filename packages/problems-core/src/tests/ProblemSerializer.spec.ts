...
describe('ProblemSerializer', () => {
  ...
  describe('fromJson', () => {
    ...
    it('should throw for non-object input', () => {
      expect(() => ProblemSerializer.fromJson('string')).toThrow('Expected object for ProblemDetails');
    ...
    it('should throw for missing required fields', () => {
      ...
      expect(() => ProblemSerializer.fromJson({ type: 'test' })).toThrow('Missing or invalid "title" field');
      expect(() => ProblemSerializer.fromJson({ type: 'test', title: 'Test' })).toThrow(
        'Missing or invalid "status" field'
      );
      expect(() => ProblemSerializer.fromJson({ type: 'test', title: 'Test', status: 400 })).toThrow(
        'Missing or invalid "code" field'
      );
    });

    it('should throw for invalid field types', () => {
      expect(() => ProblemSerializer.fromJson({ type: 123, title: 'Test', status: 400, code: 'TEST' })).toThrow(
        'Missing or invalid "type" field'
      );
      expect(() => ProblemSerializer.fromJson({ type: 'test', title: 123, status: 400, code: 'TEST' })).toThrow(
        'Missing or invalid "title" field'
      );
      expect(() => ProblemSerializer.fromJson({ type: 'test', title: 'Test', status: '400', code: 'TEST' })).toThrow(
        'Missing or invalid "status" field'
      );
      expect(() => ProblemSerializer.fromJson({ type: 'test', title: 'Test', status: 400, code: 123 })).toThrow(
        'Missing or invalid "code" field'
      );
    });
  });
