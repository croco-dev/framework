...
| `desktop [args]`                        | Report the removed desktop capability            |
...
### desktop — Removed Capability

Desktop contracts and code generation are no longer supported. Former `generate`, `check`, and
`diff` invocations fail with `CROCO_DESKTOP_REMOVED`; Croco does not replace them with a web scaffold.
See [Desktop support removal](../../docs/release/problem-code-migrations.md#desktop-support-removal)
for the repository revision to pin while migrating.
