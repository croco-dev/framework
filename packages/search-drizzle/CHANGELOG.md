...
- 45bd713: Return the full matching document count from search results, including partial and out-of-range pages. Search strategies now return a typed row-and-total query plan whose statements share the same search and tenant predicates.
...
- 8509209: Persist indexed document tenant scope only in the canonical `tenant_id` column supplied by the active context.
- 24e4a0b: Make repeated PostgreSQL document indexing update the existing tenant-scoped row instead of failing on its unique identity.
...
- 9a77db0: Return PostgreSQL relevance scores for pg_search and pg_trgm results and reject missing or invalid score data.
- 5d54fb4: declare Apache-2.0 license across all publishable package manifests and ship LICENSE in published packages
- a6948e8: Accept PostgreSQL COUNT results as decimal integer strings or bigint values while rejecting negative and unsafe totals.
...
- 4fec6ec: Bind undefined document fields as SQL null during bulk indexing while preserving omitted fields and populated values.
