...
- `search(index, query)`, 전략 SQL로 검색 결과를 반환합니다.
- `indexDocument(index, document)`, 문서를 인덱싱합니다.
- `deleteDocument(index, documentId)`, 문서를 삭제합니다.
- `bulkIndex(index, documents)`, 문서를 순차적으로 일괄 인덱싱합니다.
