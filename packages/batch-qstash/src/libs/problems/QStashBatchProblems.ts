...
export class QStashBatchConfigProblem extends Problem {
  readonly code = "batch-qstash/missing-config";
  readonly category = ProblemCategory.InternalServerError;

  constructor(configKey: string) {
    super(undefined, undefined, `Missing required QStash batch configuration: ${configKey}`, {
      extensions: {
        configKey,
        retryable: false,
      },
...
export class QStashBatchValidationProblem extends Problem {
  readonly code = "batch-qstash/invalid-publish-request";
...
export class QStashBatchPublishProblem extends Problem {
  readonly code = "batch-qstash/publish-failed";

  constructor(operation: string, error: unknown) {
    const status = getUpstreamStatus(error);
    const retryable = isRetryableQStashBatchError(error);
    const message = redactSensitiveValue(getErrorMessage(error));

    super(
      "batch-qstash/publish-failed",
      mapUpstreamProblemCategory(status, retryable),
      `QStash batch ${operation} failed: ${message}`,
      {
        extensions: {
          operation,
          provider: "qstash",
...
export function isRetryableQStashBatchError(error: unknown): boolean {
  const status = getUpstreamStatus(error);
  if (status !== undefined) {
