...
The Croco 1.0 compatibility train is the spine package set plus the Croco packages emitted into
generated app package manifests. It is intentionally validated as a train while package versions
remain independently managed by Changesets.

Current fixed/linked decision: validation is sufficient. `.changeset/config.json` keeps `fixed: []`
and `linked: []`; introduce fixed or linked groups only when release evidence shows that published
packages must be bumped in lockstep rather than checked by the compatibility train gates.

Train rules:

1. Internal `@croco/*` workspace dependencies use `workspace:*` in `dependencies`,
   `devDependencies`, `peerDependencies`, and `optionalDependencies`.
2. Published internal peer semver exceptions are peer-only and must be listed in
   `scripts/internal-peer-dependency-range-exceptions.json` with `package`, `section`,
   `dependency`, `range`, `reason`, `owner`, and `compatibilityRationale`.
3. `create-croco-app` exposes the generated app Croco version set from
   `packages/create-croco-app/src/helpers/croco-ranges.ts`; tests check the template dependency
   intersection with `docs/package-catalog.json` `spine.packages` and the current workspace package
   versions.
4. `pnpm package-quality:report` reports range drift, checked peer exceptions, spine packages,
   generated app template ranges, actual generated app semver ranges, expected workspace package
   ranges, and the fixed/linked decision in the compatibility train section.
...
| ------------------------ | ---------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Package manifest policy  | `pnpm package-manifests:check` validates package catalog, manifests, and internal Croco train ranges | Importable spine packages use source-root `main`/`types` plus dist-only `publishConfig`, unless a checked direct-dist exception applies. Internal `@croco/*` workspace ranges use `workspace:*`; peer semver exceptions require checked owner/reason/compatibility metadata.                                                           |
...
| Generated app smoke      | `pnpm create-croco-app:smoke`, `pnpm alpha-release:smoke`                                            | The golden generated app paths must exercise spine protocol, transport, CLI, and codegen packages without live third-party credentials, and the packed-artifact release smoke must prove generated apps install, build, test, and smoke outside the monorepo checkout.                                                                 |
...
| Spine promotion check    | `pnpm spine-promotion:check`                                                                         | Beta spine packages must map behavior, compatibility, and failure-recovery claims to blocking command IDs plus current-run package tests or semantically passed artifacts; prose is descriptive only.                                                                                                                                  |
...
| Bundle-size warning      | `pnpm package-quality:report`                                                                        | Spine package generated artifacts are measured against the committed bundle baseline, but findings stay advisory until the baseline is promoted to a blocking gate.                                                                                                                                                                    |
